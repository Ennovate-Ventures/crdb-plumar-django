...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
	...
	_gsScope._gsDefine("plugins.CSSPlugin", ["plugins.TweenPlugin","TweenLite"], function(TweenPlugin, TweenLite) {
		...
		var _getFormatter = function(dflt, clr, collapsible, multi) {
			 ...
			 * @return {CSSPropTween} The first CSSPropTween in the linked list which common the new one(s) added by the parseComplex() call.
			 */
		 ...
		 * @return {CSSPropTween} The first CSSPropTween in the linked list which common the new one(s) added by the parse() call.
		 */
		...
		var _transformProps = ("scaleX,scaleY,scaleZ,x,y,z,skewX,skewY,rotation,rotationX,rotationY,perspective,xPercent,yPercent").split(","),
			...
			*/
			...
			_setTransformRatio = _internals.set3DTransformRatio = _internals.setTransformRatio = function(v) {
				...
				//optimized way of concatenating all the values into a string. If we do it all in one shot, it's slower because of the way browsers have to create temp.html strings and the way it affects memory. If we do it piece-by-piece with +=, it's a bit slower too. We found that doing it in these sized chunks works best overall:
		...
		_registerComplexSpecialProp("backgroundPosition", {defaultValue:"0 0", parser:function(t, e, p, cssp, pt, plugin) {
			...
			if ((bs.indexOf("%") !== -1) !== (es.indexOf("%") !== -1) && es.split(",").length < 2) {
				...
				if (src && src !== "none") {
					ba = bs.split(" ");
					ea = es.split(" ");
					_tempImg.setAttribute("src", src); //set the temp.html IMG's src to the background-image so that we can measure its width/height
					i = 2;
					while (--i > -1) {
						bs = ba[i];
						pct = (bs.indexOf("%") !== -1);
