(function( $ ) {
			...
			$.getJSON(
				...
				function(data) {
					...
					if(data && typeof data === 'object' && data.hasOwnProperty('query') && data.query.hasOwnProperty('results') && data.query.results.hasOwnProperty('channel')) {
						...
						$this.find("span.revslider_data_weather_title").text(weather_info["title"]);
						$this.find("span.revslider_data_weather_temp").text(weather_info["item"]["condition"]["temp.html"]);
						$this.find("span.revslider_data_weather_alt_temp").text(get_alt_temp(weather_info["units"]["temperature"],weather_info["item"]["condition"]["temp.html"]));
	...
	/**
	 * Get alternative temp.html unit data
	 * @since    1.0.0
	 */
